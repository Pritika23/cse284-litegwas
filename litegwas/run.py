...
def main():
    ap = argparse.ArgumentParser(description="LiteGWAS: covariate-adjusted OLS GWAS (quantitative trait).")
    ...
    ap.add_argument("--covar", default=None, help="Optional covar.tsv with IID + covariate columns")
    ...
    # MVP assumption: geno.npy rows are in the same order as pheno.tsv IIDs
    if len(iids) != G.shape[0]:
        raise ValueError(
            f"After aligning pheno/covar, N={len(iids)} but geno.npy has N={G.shape[0]}.\n"
            "For the MVP, keep geno.npy rows in the same order as pheno.tsv IIDs."
        )
    ...
    if args.plot_prefix is not None:
        ...
        manhattan_plot(out_df, man_png, title="LiteGWAS Manhattan")
        qq_plot(out_df, qq_png, title="LiteGWAS QQ")
